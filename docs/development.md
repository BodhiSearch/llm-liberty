...
1. Create `src/providers/<name>.ts` exporting `loginXyz({ verify, example }: LoginOptions): Promise<void>`.
...
3. Reuse `src/oauth/pkce.ts` and `src/oauth/callback-server.ts`. Use `open` to launch the browser and `@clack/prompts`'s `spinner` for stderr progress.
4. Build the envelope conforming to [`docs/output-contract.md`](output-contract.md). Populate `headers` and `body` with whatever the provider's API filters on.
5. Implement a `verify` step that proves the token works end-to-end — for chat providers, list models, pick a small/cheap one, send a one-word prompt, assert the response. The `--no-verify` flag skips it.
6. Build a `curl` example with `buildCurlExample(creds, model | null)` so users can copy-paste a working request. The `--no-example` flag skips it.
7. Wire the new provider into the dispatcher in `src/cli.ts`.
